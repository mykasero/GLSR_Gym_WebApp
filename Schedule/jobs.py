...
def cleaning_user_roll(reroll, days_until_monday, scheduler):
    ...
    if date.today().strftime("%A") == "Monday" or scheduler == False:
        ...
        all_users = [user['username'] for user in User.objects.exclude(
                username='Super'
            ).values() if user['is_active']==True]
        
        # get all the users usernames from CleaningSchedule 
        # that were already picked in this set
        ...
        if len(all_users) == len(picked_users):
            ...
            user_to_archive = CleaningSchedule.objects.filter(
                username=picked_users[-1]
             )[0]
            ...
            # adding this to prevent the last picked user from the previous set 
            # to be picked instatly again since random tends to do 
            # this way too often
            user_last_picked_last_set = [
                CleaningScheduleArchive.objects.all().order_by('-id')[0].username
                ]
            # now the recent picks are empty so roll a new user
            this_weeks_user = choice(
                list(set(all_users)-set(user_last_picked_last_set))
                )
        else:
            # if there are more than 1 users in the recent picks, 
            # archive the most recent one before creating a new 
            # cleaning schedule entry
        ...
        # if admin called reroll - remove the currently picked user pick 
        # and his archive, create a new pick
        if reroll:
